...
impl SharedDcareGoogleSheet {
    pub async fn new(
        ...
        let document_id = if let Some(did) = document_id {
            did
        } else {
            return Err(SheetsError::DocumentIdError);
        };
        let tab_name = if let Some(tab) = tab_name {
            tab
        } else {
            return Err(SheetsError::TabNameError);
        };
...
mod tests {
    ...
    async fn try_create() -> Result<SharedDcareGoogleSheet, SheetsError> {
        SharedDcareGoogleSheet::new(
            None,
            Some("19cQ_zAgqkM_iqOiqECP1yVTobuRkFbwk-VfegOys8ZE".to_string()),
            Some("工單表".to_string()),
        )
        .await
    }
